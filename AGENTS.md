...
- **One-time flags belong in `startup-flags.json`, never in `store.json`.** `main` reads the store under a `.const` watch that restarts the service on any change, so clearing a consumed flag there loops — this is exactly what made Reset Wallet Transactions re-run on every restart. The flags file is read with `.once` and cleared by a oneshot.
