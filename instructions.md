...
Under **Performance**, **Graph Cache Duration** controls how long LND reuses the answer to a full network graph query. Apps that draw the Lightning network — Mempool and Ride The Lightning — re-run that query on every refresh, and each run reads the whole graph from disk. StartOS sets this to **1 minute** so repeated or simultaneous refreshes share one read instead of stalling gossip and payments; raise it if you refresh those dashboards often, or set it to 0 to read the graph fresh every time. **Stagger Initial Reconnect** is also on by default, spreading reconnections to your peers over the first 30 seconds after a restart — it does nothing until you have more than 10 channel peers.
