...
**Configure Channel Backups** closes that gap. Pick any combination of Google Drive, Dropbox, Nextcloud and SFTP, and a copy is sent there every time your channels change. The backup itself is encrypted by LND with a key derived from your seed. Its filenames and marker metadata are plaintext, however, so the storage provider can see when your channel set changes. Prefer somewhere you control, and use two targets if you can. A copy on this same server dies with it, so choose another machine; an address that plainly points at this server is refused, but a folder elsewhere on the same disk cannot be told apart from a real target.
...
Restoring asks each peer to force-close from the Static Channel Backup, and shows a persistent warning. If you configured channel backups, the restore also recovers from every copy on your storage targets, so channels opened since that StartOS backup are recovered too; targets you have disabled are searched as long as their credentials are still saved. Every distinct retained copy is tried, newest first, so a corrupt copy falls back through older generations. The restore waits until every target has answered. If one is gone for good, turn it off and use **Forget saved credentials** for it in **Configure Channel Backups** so the restore can finish; the restore notice names the target it is waiting on. If you move a target to a different folder or server, take a new StartOS backup afterwards, or a restore will look in the old place. **Lightning Labs strongly recommends against continued use of a restored node:** once funds are back on-chain, sweep them to another wallet, then uninstall and reinstall LND fresh.
