...
Restoring asks each peer to force-close from the Static Channel Backup, and shows a persistent warning. If you configured channel backups, the restore also recovers from every copy on your storage targets, so channels opened since that StartOS backup are recovered too; targets you have disabled are searched as long as their credentials are still saved. The restore waits until every target has answered. If one is gone for good, clear its saved credentials in **Configure Channel Backups** so the restore can finish; the restore notice names the target it is waiting on. If you move a target to a different folder or server, take a new StartOS backup afterwards, or a restore will look in the old place. **Lightning Labs strongly recommends against continued use of a restored node:** once funds are back on-chain, sweep them to another wallet, then uninstall and reinstall LND fresh.
