...
sshpass -p "$MYNODE_PASS" ssh -o StrictHostKeyChecking=no admin@$MYNODE_HOST "echo \"$MYNODE_PASS\" | sudo -S /usr/bin/mynode_stop_critical_services.sh"
>&2 echo "Copying LND data"
sshpass -p "$MYNODE_PASS" ssh -o StrictHostKeyChecking=no admin@$MYNODE_HOST "echo \"$MYNODE_PASS\" | sudo -S chmod -R 755 /mnt/hdd/mynode/lnd/data"
