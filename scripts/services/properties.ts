...
const nodeInfoMatcher = shape({identity_pubkey: string, alias: string, block_height: number, synced_to_chain: boolean, synced_to_graph: boolean});
const noPropertiesFound = { result: { version: 2, data: { "Not Ready": { type: "string", value: "Could not find properties. The service might still be starting", qr: false, copyable: false, masked: false, description: "Fallback message for when properties cannot be found", } }, } } as const;
const wrongShape = (wrongValue: unknown): T.ResultType<T.Properties> => ({ result: { version: 2, data: { "Wrong shape": { type: "string", value: `Called out to getinfo but the shape was wrong. This gives us the error ${nodeInfoMatcher.errorMessage(wrongValue)}`, qr: false, copyable: false, masked: false, description: "Fallback message for when properties cannot be found", } }, } }) as const;
...
export const properties: T.ExpectedExports.properties = async (effects: T.Effects) => {
    ...
    const exists = async (path: string): Promise<boolean> => await util.exists(effects, { volumeId: "main", path });
    if (!(await Promise.all(paths.map(exists))).every((v) => v)) return noPropertiesFound;
  
    const [controlTorAddress, peerTorAddress, macaroonHex, macaroonBase64URL, cert] = await Promise.all([
      ...paths.map(async (path) => (await effects.readFile({ volumeId: "main", path })).trim()),
      effects.readFile({ volumeId: "main", path: "start9/admin.macaroon.hex" }),
      effects.readFile({ volumeId: "main", path: "start9/admin.macaroon.base64url" }),
      effects.readFile({ volumeId: "main", path: "start9/control.cert.pem.base64url" }),
    ]);
  
    try {
      const nodeInfo = await effects.fetch("https://lnd.embassy:8080/v1/getinfo", { headers: { "Grpc-Metadata-macaroon": macaroonHex } });
      ...
      await effects.writeFile({path: "start9/stats.yaml", volumeId: "main", toWrite: YAML.stringify(stats), });
      return { result: stats };
    } catch (e) {
