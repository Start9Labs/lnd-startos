...
function encodeHex(character) {
    ...
    return `\\${handle}${repeat("0", length - string5.length)}${string5}`;
}
function indentString(string6, spaces) {
    const ind = repeat(" ", spaces), length = string6.length;
    let position = 0, next = -1, result = "", line;
    while(position < length){
        next = string6.indexOf("\n", position);
        if (next === -1) {
            line = string6.slice(position);
            position = length;
        } else {
            line = string6.slice(position, next + 1);
            position = next + 1;
        }
        if (line.length && line !== "\n") result += ind;
        result += line;
...
function chooseScalarStyle(string8, singleLineOnly, indentPerLevel, lineWidth, testAmbiguousType) {
    const shouldTrackWidth = lineWidth !== -1;
    let hasLineBreak = false, hasFoldableLine = false, previousLineBreak = -1, plain = isPlainSafeFirst(string8.charCodeAt(0)) && !isWhitespace(string8.charCodeAt(string8.length - 1));
    let __char, i5;
    if (singleLineOnly) {
        for(i5 = 0; i5 < string8.length; i5++){
            __char = string8.charCodeAt(i5);
        ...
        for(i5 = 0; i5 < string8.length; i5++){
            ...
            if (__char === 0x0a) {
                hasLineBreak = true;
                if (shouldTrackWidth) {
                    hasFoldableLine = hasFoldableLine || i5 - previousLineBreak - 1 > lineWidth && string8[previousLineBreak + 1] !== " ";
        ...
        hasFoldableLine = hasFoldableLine || shouldTrackWidth && i5 - previousLineBreak - 1 > lineWidth && string8[previousLineBreak + 1] !== " ";
    }
    if (!hasLineBreak && !hasFoldableLine) {
        return plain && !testAmbiguousType(string8) ? 1 : 2;
    }
    if (indentPerLevel > 9 && needIndentIndicator(string8)) {
        return 5;
    }
...
function dropEndingNewline(string9) {
    return string9[string9.length - 1] === "\n" ? string9.slice(0, -1) : string9;
}
function foldString(string10, width) {
    const lineRe = /(\n+)([^\n]*)/g;
    let result = (()=>{
        let nextLF = string10.indexOf("\n");
        nextLF = nextLF !== -1 ? nextLF : string10.length;
        lineRe.lastIndex = nextLF;
        return foldLine(string10.slice(0, nextLF), width);
    })();
    let prevMoreIndented = string10[0] === "\n" || string10[0] === " ";
    let moreIndented;
    let match;
    while(match = lineRe.exec(string10)){
...
function escapeString(string11) {
    let result = "";
    let __char, nextChar;
    let escapeSeq;
    for(let i6 = 0; i6 < string11.length; i6++){
        __char = string11.charCodeAt(i6);
        if (__char >= 0xd800 && __char <= 0xdbff) {
            nextChar = string11.charCodeAt(i6 + 1);
        ...
        result += !escapeSeq && isPrintable(__char) ? string11[i6] : escapeSeq || encodeHex(__char);
    }
    return result;
}
function blockHeader(string12, indentPerLevel) {
    const indentIndicator = needIndentIndicator(string12) ? String(indentPerLevel) : "";
    const clip = string12[string12.length - 1] === "\n";
    const keep = clip && (string12[string12.length - 2] === "\n" || string12 === "\n");
    const chomp = keep ? "+" : clip ? "" : "-";
    return `${indentIndicator}${chomp}\n`;
}
function writeScalar(state, string13, level, iskey) {
    state.dump = (()=>{
        if (string13.length === 0) {
            return "''";
        }
        if (!state.noCompatMode && DEPRECATED_BOOLEANS_SYNTAX.indexOf(string13) !== -1) {
            return `'${string13}'`;
        }
        ...
        switch(chooseScalarStyle(string13, singleLineOnly, state.indent, lineWidth, testAmbiguity)){
            case STYLE_PLAIN:
                return string13;
            case STYLE_SINGLE:
                return `'${string13.replace(/'/g, "''")}'`;
            case STYLE_LITERAL:
                return `|${blockHeader(string13, state.indent)}${dropEndingNewline(indentString(string13, indent))}`;
            case STYLE_FOLDED:
                return `>${blockHeader(string13, state.indent)}${dropEndingNewline(indentString(foldString(string13, lineWidth), indent))}`;
            case STYLE_DOUBLE:
                return `"${escapeString(string13)}"`;
            default:
...
const { shape: shape1 , number: number1 , string: string1 , boolean: __boolean1  } = matches;
const matchTor = shape1({
    "use-tor-only": __boolean1,
    "stream-isolation": __boolean1
});
const matchBitcoind = shape1({
    type: string1,
    user: string1,
    password: string1
});
const matchAdvanced = shape1({
    "min-confirmations": number1,
    "confirmation-target": number1
});
const matchAutopilot = shape1({
    enabled: __boolean1,
    private: __boolean1,
    maxchannels: number1,
    allocation: number1,
    "min-channel-size": number1,
    "max-channel-size": number1,
    advanced: matchAdvanced
});
const matchBitcoin = shape1({
    "default-channel-confirmations": number1,
    "min-htlc": number1,
    "min-htlc-out": number1,
    "base-fee": number1,
    "fee-rate": number1,
    "time-lock-delta": number1
});
const matchAdvanced2 = shape1({
    "debug-level": string1,
    "db-bolt-no-freelist-sync": __boolean1,
    "db-bolt-auto-compact": __boolean1,
    "db-bolt-auto-compact-min-age": number1,
    "db-bolt-db-timeout": number1,
    "recovery-window": number1,
    "payments-expiration-grace-period": number1,
    "default-remote-max-htlcs": number1,
    "max-channel-fee-allocation": number1,
    "max-commit-fee-rate-anchors": number1,
    "protocol-wumbo-channels": __boolean1,
    "protocol-no-anchors": __boolean1,
    "protocol-disable-script-enforced-lease": __boolean1,
    "gc-canceled-invoices-on-startup": __boolean1,
    bitcoin: matchBitcoin
}, [
    'recovery-window'
]);
const matchRoot = shape1({
    alias: string1,
    color: string1,
    "accept-keysend": __boolean1,
    "accept-amp": __boolean1,
    "reject-htlc": __boolean1,
    "min-chan-size": number1,
    "max-chan-size": number1,
    tor: matchTor,
    bitcoind: matchBitcoind,
    autopilot: matchAutopilot,
    advanced: matchAdvanced2,
    "control-tor-address": string1,
    "peer-tor-address": string1,
    "watchtower-tor-address": string1
}, [
    'alias',
    'min-chan-size',
    'max-chan-size'
]);
const { string: string2 , boolean: __boolean2 , shape: shape2 , number: number2  } = matches;
const configRules = [
    {
        currentError (config) {
...
const setConfig = async (effects, input)=>{
    ...
    const dependsOn = config.bitcoind.type === 'internal' || config.bitcoind.type === 'internal-proxy' ? {
        'bitcoind': []
    } : {};
    const result = {
        signal: "SIGTERM",
    ...
    bitcoind: {
        async check (_effects, configInput) {
            ...
            if (!config['zmq-enabled']) {
                return {
                    error: 'Must have ZeroMQ enabled'
                };
            }
            return {
                result: null
...
const { any: any1 , string: string4 , dictionary: dictionary1  } = matches;
