...
HEALTH_CHECK_SRC := $(shell find ./health-checks/src) health-check/Cargo.toml health-check/Cargo.lock
...
lnd.s9pk: manifest.yaml assets/compat/config_spec.yaml assets/compat/config_rules.yaml image.tar instructions.md LICENSE icon.png $(ASSET_PATHS)
	embassy-sdk pack

image.tar: Dockerfile docker_entrypoint.sh configurator/target/aarch64-unknown-linux-musl/release/configurator health-check $(LND_GIT_FILE)
