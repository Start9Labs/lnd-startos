...
export const current = VersionInfo.of({
  ...
  releaseNotes: {
    ...
    fr_FR:
      "Met à jour LND vers 0.21.0-beta et fait passer la base de données de l'ancien backend bolt à SQLite, supprimant la lente étape de compactage au démarrage. La conversion unique s'exécute automatiquement au premier démarrage de LND après la mise à jour et est irréversible — sauvegardez votre nœud avant de mettre à jour. En cas d'échec, LND ne démarrera pas, ce qui vous permet de réessayer. Ce premier démarrage peut prendre un certain temps, car LND convertit aussi vers le SQL natif. Cette mise à jour corrige aussi les connexions REST et gRPC depuis des portefeuilles externes : ces ports sont désormais transmis directement afin que le propre certificat TLS de LND soit utilisé de bout en bout, et l'URL de connexion REST inclut maintenant ce certificat — ainsi des portefeuilles comme Zeus se connectent de manière fiable via Tor et le réseau local.",
  },
  migrations: {
    up: async ({ effects }) => {
