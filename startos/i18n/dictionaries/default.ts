...
const dict = {
  ...
  'Keep a current copy of channel.backup on a storage provider, so a StartOS restore recovers channels opened after that backup was taken. A supplement to StartOS backups, not a replacement: only a StartOS backup holds the wallet, and only a StartOS restore uses these copies.': 311,
  'channel.backup is encrypted by LND under a key derived from your wallet seed. The storage provider can still see when it is updated. Use a target on a different machine, and prefer two independent targets. Tor .onion targets are not supported yet. Keep taking StartOS backups: a copy here is used only by a StartOS restore.': 312,
  ...
  'Channel Backups': 372,
  'channel.backup will be copied to ${targets} whenever your channels change, and a StartOS restore will recover channels from those copies together with its own. Run Back Up Channels Now to check that it works.': 373,
  ...
  'A StartOS backup only holds the channels you had when you took it. An off-server copy stays current, and a StartOS restore recovers from both.': 379,
  'Channel Backup': 380,
