...
That state is indistinguishable from a large legitimate backfill through `getinfo` alone, so the message reports what can be distinguished: no peers connected, the peer count while the wait is still normal, and — past fifteen minutes — how long the sync has been pending. **The result stays `loading` in every one of those cases**, so nothing about the wait changes what dependent services see.
