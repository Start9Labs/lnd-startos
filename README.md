...
`lndinit` is added solely for the offline bolt → SQLite database conversion (see [Database backend](#database-backend)); the `lnd` binary and runtime are otherwise the upstream image. The Dockerfile also restores `curl` (dropped when v0.21 moved the upstream image to Alpine) and adds `sqlite`, `openssh-client`, and `sshpass` — see [Initialize Wallet](#initialize-wallet) for what the SSH pair is for.
...
| ------- | -------------------------- | -------- | ------------------------------------------------ | --------------------------------- |
| Umbrel  | `assets/import-umbrel.sh`  | `umbrel` | `~/umbrel/app-data/lightning/data/lnd/`          | Fixed upstream constant           |
...
myNode and StartOS stream `data/` through `tar` over the SSH session; Umbrel `scp`s the whole LND directory. Either way `lnd.conf` and the TLS pair are StartOS's own — the file model reparses `lnd.conf` through its schema (dropping any key it doesn't declare) and `setupCerts` rewrites the certificate on every init, so nothing an origin's config carries survives into the running node. On myNode, `admin` cannot read `/mnt/hdd/mynode/lnd` without `sudo` and its password is the same one given for SSH; piping it to `sudo -S` over the command's stdin keeps it out of the remote process list, and `tar -o` drops myNode's `bitcoin:bitcoin` ownership on the way in. A StartOS origin is vetted before anything is stopped or copied: if its `startup-flags.json` reports `dbSchemaFinalized` without `dbMigrationComplete` the origin was caught mid bolt → SQLite conversion with a half-converted channel database, and the import is refused (an origin with no `startup-flags.json` predates the conversion, is plain `bolt`, and proceeds).
